...
Foam::pureChemistryModel<ReactionThermo, ThermoType>::pureChemistryModel
(
...
:
    ...
    c_(nSpecie_),
    dcdt_(nSpecie_),
    optimizedODE_(this->template lookupOrDefault<Switch>("optimizedODE", false)),
    fastChemistryPtr_(
        basicFastChemistryModel::New(this->mesh())
    )

{
    if (optimizedODE_)
    {
        Info << "Using optimized ODE solver" << endl;
        GetRRFunc_ = &pureChemistryModel<ReactionThermo,ThermoType>::getRRGivenYTP_Optimized;
    }
    else
    {
        Info << "Using standard ODE solver" << endl;
        GetRRFunc_ = &pureChemistryModel<ReactionThermo,ThermoType>::getRRGivenYTP_Basic;
    }
    ...
    Info<< "pureChemistryModel: Number of species = " << nSpecie_
...
template<class ReactionThermo, class ThermoType>
Foam::pureChemistryModel<ReactionThermo, ThermoType>::
~pureChemistryModel()
{}
...
template<class ReactionThermo, class ThermoType>
void Foam::pureChemistryModel<ReactionThermo, ThermoType>::omega
(
    const scalarField& c,
    const scalar T,
    const scalar p,
...
template<class ReactionThermo, class ThermoType>
Foam::scalar Foam::pureChemistryModel<ReactionThermo, ThermoType>::omegaI
(
    const label index,
    const scalarField& c,
    const scalar T,
...
template<class ReactionThermo, class ThermoType>
void Foam::pureChemistryModel<ReactionThermo, ThermoType>::derivatives
(
    const scalar time,
    const scalarField& c,
    scalarField& dcdt
...
template<class ReactionThermo, class ThermoType>
void Foam::pureChemistryModel<ReactionThermo, ThermoType>::jacobian
(
    const scalar t,
    const scalarField& c,
    scalarField& dcdt,
...
Foam::pureChemistryModel<ReactionThermo, ThermoType>::tc() const
{
...
template<class ReactionThermo, class ThermoType>
Foam::tmp<Foam::volScalarField>
Foam::pureChemistryModel<ReactionThermo, ThermoType>::Qdot() const
{
...
Foam::pureChemistryModel<ReactionThermo, ThermoType>::calculateRR
(
    const label ri,
    const label si
) const
...
template<class ReactionThermo, class ThermoType>
void Foam::pureChemistryModel<ReactionThermo, ThermoType>::calculate()
{
    if (!this->chemistry_)
    {
        return;
...
Foam::scalar Foam::pureChemistryModel<ReactionThermo, ThermoType>::solve
(
...
)
{
    ...
    forAll(rho, celli)
    {
        ...
        scalarField RR = (this->*GetRRFunc_)(Yi,Ti,pi,deltaT[celli],this->deltaTChem_[celli],rhoi);
...
template<class ReactionThermo, class ThermoType>
Foam::scalarField Foam::pureChemistryModel<ReactionThermo, ThermoType>::getRRGivenYTP
(
    const scalarField& Y,
    scalar& T,
    scalar& p,
...
)
{
    ...
    if (T > Treact_)
    {
        const scalar rhoi = rho;
        scalar pi = p;
        scalar Ti = T;

        for (label i=0; i<nSpecie_; i++)
        {
            c_[i] = rhoi*Y[i]/specieThermo_[i].W();
            c0[i] = c_[i];
        }
        ...
        {
            scalar dt = timeLeft;
            this->solve(c_, Ti, pi, dt, deltaTChem);
            timeLeft -= dt;
        }

        //scalar deltaTMin = min(deltaTChem, great);
        deltaTChem = min(deltaTChem, this->deltaTChemMax_);
...
template<class ReactionThermo, class ThermoType>
Foam::scalarField Foam::pureChemistryModel<ReactionThermo, ThermoType>::getRRGivenYTP_Basic
(
    scalarField& Y,
    scalar& T,
    scalar& p,
...
{
    return this->getRRGivenYTP(Y, T, p, deltaT, deltaTChem, rho);
}

template<class ReactionThermo, class ThermoType>
Foam::scalarField Foam::pureChemistryModel<ReactionThermo, ThermoType>::getRRGivenYTP_Optimized
(
    scalarField& Y,
    scalar& T,     
    scalar& p,
...
{
    return fastChemistryPtr_->getRRGivenYTP(Y, T, p, deltaT, deltaTChem, rho, rho);
}
